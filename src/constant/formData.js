export const EDUCATION_LEVELS = [
  { value: "slc", label: "SLC / SEE (Class 10)" },
  { value: "plus2", label: "+2 / Intermediate (Class 12)" },
  { value: "bachelors", label: "Bachelor's Degree" },
  { value: "masters", label: "Master's Degree" },
];

export const FACULTIES = {
  slc: [{ value: "general", label: "General" }],
  plus2: [
    { value: "science", label: "Science" },
    { value: "management", label: "Management" },
    { value: "humanities", label: "Humanities" },
    { value: "education", label: "Education" },
  ],
  bachelors: [
    { value: "engineering", label: "Engineering / Technical" },
    { value: "medicine", label: "Medicine / Health" },
    { value: "law", label: "Law" },
    { value: "management", label: "Management / Commerce" },
    { value: "science", label: "Science & Technology" },
    { value: "humanities", label: "Humanities & Social Science" },
    { value: "education", label: "Education" },
    { value: "agriculture", label: "Agriculture / Forestry" },
  ],
  masters: [
    { value: "engineering", label: "Engineering / Technical" },
    { value: "medicine", label: "Medicine / Health" },
    { value: "law", label: "Law" },
    { value: "management", label: "Management / Commerce" },
    { value: "science", label: "Science & Technology" },
    { value: "humanities", label: "Humanities & Social Science" },
    { value: "education", label: "Education" },
    { value: "agriculture", label: "Agriculture / Forestry" },
  ],
};

export const CATEGORIES = ["General", "Janajati", "Dalit", "Disabled"];

export const STATS = [
  { value: "12+", label: "Loksewa Posts" },
  { value: "100%", label: "Free Tool" },
  { value: "Instant", label: "Results" },
  { value: "Mobile", label: "Optimized" },
];

export const CATEGORY_COLORS = {
  "Officer Level": {
    text: "#4f9eff",
    bg: "rgba(79,158,255,0.1)",
    border: "rgba(79,158,255,0.3)",
  },
  "Senior Officer Level": {
    text: "#9b59b6",
    bg: "rgba(155,89,182,0.1)",
    border: "rgba(155,89,182,0.3)",
  },
  "Technical / Engineering": {
    text: "#f39c12",
    bg: "rgba(243,156,18,0.1)",
    border: "rgba(243,156,18,0.3)",
  },
  "Health / Medical": {
    text: "#2ecc71",
    bg: "rgba(46,204,113,0.1)",
    border: "rgba(46,204,113,0.3)",
  },
  Agriculture: {
    text: "#27ae60",
    bg: "rgba(39,174,96,0.1)",
    border: "rgba(39,174,96,0.3)",
  },
  "Revenue / Finance": {
    text: "#e67e22",
    bg: "rgba(230,126,34,0.1)",
    border: "rgba(230,126,34,0.3)",
  },
  "Education / Teaching": {
    text: "#3498db",
    bg: "rgba(52,152,219,0.1)",
    border: "rgba(52,152,219,0.3)",
  },
  "Security / Police": {
    text: "#e74c3c",
    bg: "rgba(231,76,60,0.1)",
    border: "rgba(231,76,60,0.3)",
  },
  "Security / Military": {
    text: "#c0392b",
    bg: "rgba(192,57,43,0.1)",
    border: "rgba(192,57,43,0.3)",
  },
  "Environment / Forest": {
    text: "#1abc9c",
    bg: "rgba(26,188,156,0.1)",
    border: "rgba(26,188,156,0.3)",
  },
  "Sub-register Level": {
    text: "#95a5a6",
    bg: "rgba(149,165,166,0.1)",
    border: "rgba(149,165,166,0.3)",
  },
  "Section Officer Level": {
    text: "#7f8c8d",
    bg: "rgba(127,140,141,0.1)",
    border: "rgba(127,140,141,0.3)",
  },
};

export const EDU_LABELS = {
  slc: "SLC/SEE",
  plus2: "+2",
  bachelors: "Bachelor's",
  masters: "Master's",
};

export const inputBase =
  "w-full bg-[#0e0e0e] border border-[#2a2a2a] rounded-xl text-white text-[15px] px-4 py-[14px] outline-none transition-colors duration-200 focus:border-[#e8294a] placeholder-[#444]";

export const selectBase =
  "appearance-none w-full bg-[#0e0e0e] border border-[#2a2a2a] rounded-xl text-white text-[15px] px-4 py-[14px] pr-10 outline-none transition-colors duration-200 focus:border-[#e8294a] cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed";
